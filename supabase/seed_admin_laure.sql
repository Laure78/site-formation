...
-- Prérequis : l'utilisateur laureolivie@yahoo.fr doit avoir créé un compte
...
BEGIN
  ...
  SELECT id INTO uid FROM auth.users WHERE email = 'laureolivie@yahoo.fr' LIMIT 1;

  IF uid IS NULL THEN
    RAISE NOTICE 'Utilisateur laureolivie@yahoo.fr non trouvé. Inscrivez-vous d''abord sur le site ou créez l''utilisateur dans Supabase > Auth > Users.';
    RETURN;
  END IF;
