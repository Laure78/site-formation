...
} from '@/app/mon-espace/agenda/actions';
import {
  toInputDateValue,
  toInputTimeValue,
...
type Props = {
  open: boolean;
  onClose: () => void;
  /** Préremplissage date (YYYY-MM-DD) pour un nouvel événement */
...
export function AgendaEventFormDialog({
  ...
  return (
    ...
    >
      ...
      >
        <h2
          id="agenda-event-form-title"
          className="font-display text-lg font-semibold text-slate-900"
        >
          {isEdit ? 'Modifier l’événement' : 'Nouvel événement'}
        </h2>
        <p className="mt-1 text-sm text-slate-500">
          Titre, date, horaires et catégorie — description optionnelle.
        </p>
        ...
        <form
          ...
          action={(formData) => {
            ...
            startTransition(async () => {
              try {
                if (isEdit) await updateAgendaEventAction(formData);
                else await createAgendaEventAction(formData);
                onClose();
              } catch (err) {
        ...
        >
          ...
          <label className="block text-xs font-semibold uppercase tracking-wide text-slate-400">
            ...
            <input
              ...
              placeholder="Ex. Session formation"
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-400 sm:col-span-1">
              Date
              <input
                type="date"
                name="date"
            ...
            {isEdit && event ? (
              <button
                ...
                onClick={() => {
                  ...
                  fd.set('id', event.id);
                  startTransition(async () => {
                    await deleteAgendaEventAction(fd);
                    onClose();
                  });
                }}
              >
