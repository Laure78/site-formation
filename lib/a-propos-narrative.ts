...
export const A_PROPOS_NARRATIVE_PARAGRAPHS: readonly string[] = [
  ...
  `Côté visibilité et transmission, j’assume aussi un rôle d’instructrice sur LinkedIn Learning : deux cours en français sur l’IA appliquée au BTP et aux TPE, pour toucher des publics au-delà de l’Île-de-France. Côté institutions, les collaborations et interventions s’appuient sur des acteurs de référence — FFB (dont FFB Grand Paris), CSFE, CNAM Entreprise, Lefebvre Dalloz, CAPEB — parce que la crédibilité d’une formatrice IA pour le BTP se joue aussi dans la capacité à parler le langage des corps de métier et des fédérations.`,
