...
export const LAURE_OLIVIE_STATS_FRESHNESS = 'au 17 avril 2026' as const;
...
export const LAURE_OLIVIE_PARCOURS_TIMELINE = [
  ...
  {
    period: '2026',
    title: `${formatProfessionalsTrainedCount()} professionnels formés · ${SOCIAL_PROOF.AVERAGE_RATING}`,
...
export function getLaureOlivieFormationPortraitParagraph(contextLine?: string): string {
  const base = `Formatrice IA générative spécialiste BTP depuis 2022, Laure Olivié cumule une expérience de chargée de formation au CNFPT (2009-2019), de dirigeante d'ALIA BTP (2017-2024) et d'instructrice LinkedIn Learning. Elle a formé ${formatProfessionalsTrainedCount()} professionnels du bâtiment et des travaux publics (${SOCIAL_PROOF.AVERAGE_RATING}, ${LAURE_OLIVIE_STATS_FRESHNESS}) sur ChatGPT, Claude, Copilot et Mistral — devis, DCE, CCTP, CR chantier et mémoires techniques. OFC Création d'Entreprise est certifié Qualiopi (NDA ${SCHEMA_CONTACT.nda}), finançable Constructys selon éligibilité.`;
...
export function getLaureOlivieArticleAuthorBio(): string {
  return `Après dix ans de conduite de travaux et de direction d'ALIA BTP (travaux publics, Île-de-France), Laure Olivié forme les équipes du BTP à l'IA générative depuis 2022. Master Stratégie d'entreprise (CNAM, 2021), instructrice LinkedIn Learning, ${formatProfessionalsTrainedCount()} pros formés · ${SOCIAL_PROOF.AVERAGE_RATING}. Références : ${LAURE_OLIVIE_CLIENT_REFERENCES_SHORT}.`;
}

/** Bio auteure fiches formation — angle crédibilité & lien /a-propos. */
export function getLaureOlivieAuthorBioBody(): string {
  return `est formatrice IA générative spécialiste BTP et fondatrice d'OFC Création d'Entreprise, organisme certifié Qualiopi (NDA ${SCHEMA_CONTACT.nda}). Parcours : CNFPT (formation e-learning, 2009-2019), dirigeante ALIA BTP (2017-2024), instructrice LinkedIn Learning. ${formatProfessionalsTrainedCount()} professionnels formés · ${SOCIAL_PROOF.AVERAGE_RATING} (${LAURE_OLIVIE_STATS_FRESHNESS}). Outils enseignés : ${LAURE_OLIVIE_IA_TOOLS.slice(0, 4).join(', ')}.`;
}

/** Description JSON-LD Person — schémas globaux. */
export function getLaureOlivieSchemaPersonDescription(): string {
  return `Formatrice IA générative spécialiste BTP. Ex-dirigeante ALIA BTP (2017-2024), ex-chargée de formation CNFPT (2009-2019). Master CNAM Stratégie d'entreprise (2021). Instructrice LinkedIn Learning. ${formatProfessionalsTrainedCount()} professionnels formés, ${SOCIAL_PROOF.AVERAGE_RATING}. Qualiopi. Clients : ${LAURE_OLIVIE_CLIENT_REFERENCES_SHORT}.`;
}

/** Intro E-E-A-T page /a-propos — angle mission & double compétence. */
export function getLaureOlivieEeatIntro(): string {
  return `Je suis Laure Olivié, formatrice IA spécialisée bâtiment depuis 2022 — après dix ans de conduite de travaux et de direction d'ALIA BTP, et une première expérience de chargée de formation au CNFPT (2009-2019). J'ai formé ${formatProfessionalsTrainedCount()} professionnels du BTP à utiliser l'IA sur leurs tâches réelles : comptes rendus, devis, appels d'offres, DCE, DOE, PPSPS et coordination chantier. Mon approche croise le terrain BTP, la pédagogie adulte et la certification Qualiopi — pas la tech pour la tech.`;
}
