...
| **Ind. 31** — réclamations | Oui — `/reclamations` (sans médiateur consommation) | `app/reclamations/page.tsx` ; `lib/qualiopi-info.ts` (`QUALIOPI_RECLAMATIONS`) | **amélioration** | Constant `delaiAccuseReception: '48 h ouvrées'` non repris distinctement à l’écran (AR + réponse fusionnés sous 15 j) |
